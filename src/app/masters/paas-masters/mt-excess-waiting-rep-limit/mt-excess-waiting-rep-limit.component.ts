...
@Component({
  ...
  templateUrl: './mt-excess-waiting-rep-limit.component.html',
  styleUrls: ['./mt-excess-waiting-rep-limit.component.scss']
})
export class MtExcessWaitingRepLimitComponent implements OnInit {
  ...
  doSearch(data) {
    ...
    this.service.post_service(ApiserviceService.apisList.getMtExcessWaitingRepLimit, data).subscribe(response => {
  ...
  saveForm(form: NgForm) {
    ...
    console.log("submit data:  " + JSON.stringify(data));
    this.service.post_service(ApiserviceService.apisList.uploadMtExcessWaitingRepLimit, data).subscribe(response => {
      //console.log("response array string: "+JSON.stringify(response));
  ...
  updateForm(form: NgForm) {
    ...
    this.service.post_service(ApiserviceService.apisList.updateMtExcessWaitingRepLimit, data).subscribe(response => {
    ...
    if (this.responseItemsList.length > 0) {
      ...
      this.service.post_service(ApiserviceService.apisList.getMtExcessWaitingRepLimit, data).subscribe(response => {
        ...
        if (response['statusCode'] == 200) {
          ...
          XLSX.writeFile(workBook, 'Excess Waiting Rep Limit.xlsx');
          this.isLoading = false;
        } else {
          this.toastr.error(response['message'] ? response['message'] : 'Something went wrong...', 'Error!');
          this.isLoading = false;
